    ...
    <q-page class=" q-pa-lg justify-center items-center">
        <div class="q-pa-lg">
            ...
            <q-table style="border:1px transparent ;border-radius: 20px;" :rows="rows" :columns="columns"
                separator='none' v-model:pagination="pagination" :loading="loading" rounded :filter="filter"
                ...
                <template v-slot:top>
                    ...
                    <q-input v-if="type=='text'" class="col-6" dense densdebounce="300" v-model="filter"
                        placeholder="Recherche" rounded outlined color="light-blue-9">
                        <template v-slot:prepend>
                            <q-icon name="search" color="grey-5" />
                        </template>
                    </q-input>
                    <q-input v-else class="col-6" dense densdebounce="300" type="date" v-model="filter"
                        placeholder="Recherche" rounded outlined color="light-blue-9">
                        <template v-slot:prepend>
                            <q-icon name="search" color="grey-5" />
                        </template>
                    ...
                    <q-select class="col-4 " :popup-content-style="{ color: 'grey' }" color="light-blue-9"
                        chips-color="yellow" rounded outlined dense v-model="filterBy" :options="options"
                ...
                <template v-slot:header="props">
                    <q-tr :props="props">
                        <q-th v-for="col in props.cols" :key="col.name" :props="props" @click="sortByColumn(col.name)"
                            :class="{'hidden':(col.name=='date')}" style="font-family:Georgia; font-weight: bold;
                            background-color:rgb(244,246,250) ;
                            color: rgb(116,124,128) ;">
                            {{ col.label }}

                            <span v-if="
                            sortColumn == col.name && col.name=='amount'"
...
export default {
    ...
    setup() {
        ...
        async function fetchData(page, sort) {
            ...
            await getHistoryIncomes('incomes/history', page + '/', filterBy.value.value, filter.value, sort).then(res => {
                if (res.status == 200) {
        ...
        return {
            pagination, arrowIconName, sortColumn,
