    ...
    <q-dialog v-model="confirm" persistent>
      <q-card class="bg-light-blue-10">
        <q-card-section class="row items-center">
          ...
          <span class="q-ml-sm text-white">Attention!La suppresion d'un type menera à la suppression <br /> des revenus
            et depenses liées à ce dernier..</span>
        </q-card-section>
  ...
  setup() {
    ...
    const createOrUpdate = function () {
      ...
      if (!v$.value.$error) {
        ...
        if (update.value) {
          updateObject('types', state.value, id.value).then((res) => {
            if (res.status == 200) {
          ...
          createObject('types', state.value).then((res) => {
