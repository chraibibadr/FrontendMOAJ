            ...
            <template v-slot:top> 
             <q-input class="col-6" dense densdebounce="300"  v-model="filter" placeholder="Recherche" rounded outlined color="light-blue-9">
                ...
                <q-card class="bg-light-blue-10"  >
                    ...
                    <span class="q-ml-sm text-white">Attention!La suppresion d'une fonction menera à la suppression <br/> des utlisateurs liée à cette derniere..</span>
                    </q-card-section>
...
// create
     const createOrUpdate= function(){
       v$.value.$validate();
       if(!v$.value.$error ){
      if(update.value){  updateObject('functions',state.value,id.value).then((res)=>{
       if(res.status==200){
      ...
      else { createObject('functions',state.value).then((res)=>{
...
<style>
.my-badge-class{
  background-color:rgb(34, 95, 185);
}
.error{
